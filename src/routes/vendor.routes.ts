...
// GET /api/vendors - List all vendors in user's company database
...
// GET /api/vendors/:id - Get single vendor from company database
vendorRoutes.get("/:id", async (c) => {
  const user = c.get("user");
  const id = c.req.param("id");
  const vendor = await getService().getVendor(user.companyId!, id);
...
vendorRoutes.get("/:id/document", async (c) => {
  ...
  const result = await getService().getLatestDocument(user.companyId!, vendorId);
...
vendorRoutes.get("/:id/document/metadata", async (c) => {
  ...
  const metadata = await getService().getLatestDocumentMetadata(user.companyId!, vendorId);
  return c.json(successResponse(metadata));
});

// POST /api/vendors - Create vendor in company database
vendorRoutes.post("/", async (c) => {
  const user = c.get("user");
  const body = await c.req.json();
  const input = createVendorSchema.parse(body);

  const vendor = await getService().createVendor(user.companyId!, input);

  return c.json(successResponse(vendor), 201);
});

// PUT /api/vendors/:id - Update vendor in company database
vendorRoutes.put("/:id", async (c) => {
  ...
  const vendor = await getService().updateVendor(user.companyId!, id, input);

  return c.json(successResponse(vendor));
});

// DELETE /api/vendors/:id - Delete vendor from company database (cascade deletes in company DB)
vendorRoutes.delete("/:id", async (c) => {
  const user = c.get("user");
  const id = c.req.param("id");

  await getService().deleteVendor(user.companyId!, id);

  return c.json(successResponse({ message: "Vendor deleted successfully" }));
});

// ========== PERMISSION MANAGEMENT (OPTIONAL / SUDO) ==========
